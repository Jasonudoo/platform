...
class EventBookingViewEvent extends JViewLegacy
{
	...
	{		
		...
		$item = $this->get('Event');
		$config = EventBookingHelper::getConfig();
		if (version_compare(JVERSION, '3.0', 'ge') || $loadBootstrapCss === '0') 
		...
		$lists['early_bird_discount_type'] = JHTML::_('select.genericlist', $options, 'early_bird_discount_type', '', 'value', 'text', 
		...
		$lists['event_end_date_minute'] = JHTML::_('select.integerlist', 0, 60, 5, 'event_end_date_minute', ' class="inputbox" ', $selectedMinute, 
			'%02d');
