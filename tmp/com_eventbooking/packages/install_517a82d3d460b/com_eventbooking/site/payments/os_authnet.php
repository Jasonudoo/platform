...
class os_authnet extends os_payment
{
	...
	function processPayment($row, $data)
	{
		...
		if ($this->approved)
		{
			...
			$mainframe->redirect(JRoute::_('index.php?option=com_eventbooking&view=complete&registration_code=' . $row->registration_code . '&Itemid=' . $Itemid, false, false));
			return true;
		}
		else
		{
			$_SESSION['reason'] = $this->getResponseText();
			$mainframe->redirect(JRoute::_('index.php?option=com_eventbooking&view=failure&id=' . $row->id . '&Itemid=' . $Itemid, false, false));
			return false;
		}
	}
