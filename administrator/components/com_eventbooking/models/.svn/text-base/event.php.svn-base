...
class EventBookingModelEvent extends JModelLegacy
{
	...
	{	
		...
		if ($_FILES['thumb_image']['name']) {
			$fileExt = JString::strtoupper(JFile::getExt($_FILES['thumb_image']['name'])); 
			$supportedTypes = array('JPG', 'PNG', 'GIF');
			if (in_array($fileExt, $supportedTypes)) {
				if (JFile::exists(JPATH_ROOT.'/media/com_eventbooking/images/'.JString::strtolower($_FILES['thumb_image']['name']))) {
					$fileName = time().'_'.JString::strtolower($_FILES['thumb_image']['name']);
				} else {
					$fileName = JString::strtolower($_FILES['thumb_image']['name']);
				}								
	...
	function _storeRecurringEvent($data) {				
		...
		if (count($eventDates) == 0) {
			...
			$mainframe->redirect('index.php?option=com_eventbooking&view=events', JText::_('Invalid recurring setting'));
		} else {
	...
	{		
		...
		$this->_db->setQuery($sql) ;
		if (version_compare(JVERSION, '3.0', 'ge')) {
			$cid = array_merge($cid, $this->_db->loadColumn()) ;
		} else {
