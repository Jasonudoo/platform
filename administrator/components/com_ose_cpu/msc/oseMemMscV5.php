...
class oseUser2MscV5 extends oseUser2Instance
{
	...
	function join($id,$amount =1 ,$entry_type = 'msc',$entry_option = null)
	{
		...
		$query = " UPDATE `#__osemsc_member`"
				." SET `expired_date` = '{$this->expired_date}'"
				." WHERE `msc_id` = '{$msc_id}' AND `member_id` = '{$user_id}'"
				;
		$db->setQuery($query);
		oseDB::query();
	...
	function generateOrder($msc_id,$msc_option=null)
	{
		...
		//$paymentInfos = oseCall('msc')->getExtInfo($msc_id,'payment','obj');
		//$paymentInfo = oseGetValue($paymentInfos,$msc_option);
$db = oseDB::instance();
		$query = "SELECT * FROM `#__osemsc_ext`"
				." WHERE `type` = 'payment' AND `id` = '{$msc_id}'"
				;
		$db->setQuery($query);
