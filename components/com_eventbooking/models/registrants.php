 ...
 * @since 1.5
 */
class EventBookingModelRegistrants extends JModelLegacy
{
	/**
	...
	function _buildContentWhere()
	{
		...
		if ($search) {
			$where[] = '(LOWER(a.first_name) LIKE '.$db->Quote( '%'.$db->escape( $search, true ).'%', false ).' OR LOWER(a.last_name) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false ).' OR LOWER(a.transaction_id) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false ).') ';
		}
		...
		if (EB_ONLY_SHOW_REGISTRANTS_OF_EVENT_OWNER) {
			...
			$where[] = ' a.event_id IN (SELECT id FROM #__eb_events WHERE created_by ='.$userId.') ';	
		} 				
